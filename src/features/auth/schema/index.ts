...
export const createLoginSchema = (t: TranslationFunction) => {
	return z.object({
		email: z.email(t('auth.login.invalid_email_format')).nonempty(t('auth.login.email_is_required')),
		password: z
			.string()
			.nonempty(t('auth.login.password_is_required'))
			.min(8, t('auth.login.password_must_be_at_least_8_characters_long'))
			.regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^a-zA-Z0-9]).+$/, t('auth.login.invalid_password_format')),
	});
};
	...
	return z
		.object({
			...
			password: z
				...
				.min(8, t('auth.login.password_must_be_at_least_8_characters_long'))
				.regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^a-zA-Z0-9]).+$/, t('auth.login.invalid_password_format')),
			confirmPassword: z.string().nonempty(t('auth.register.confirm_password_is_required')),
			name: z
				.string()
